...
def calculate_match_score(resume_text, resume_skills, job_description, job_title):
    ...
    final_score = max(final_score, 0.4) if matching_skills or semantic_similarity > 0.3 else final_score
    
    return {
        'score': round(min(final_score * 100, 95), 1),  # Cap at 95%
        'semantic_similarity': round(boosted_semantic * 100, 1),
        'skill_match': round(boosted_skill * 100, 1),
    ...
    <body>
        <div class="container">
            ...
            <div class="tech-info">
                ...
                <p><strong>Embedding Model:</strong> all-MiniLM-L6-v2 (Sentence Transformers)</p>
                <p><strong>Similarity Metric:</strong> Cosine similarity between resume and job description embeddings</p>
                <p><strong>Scoring Algorithm:</strong> 60% semantic similarity + 40% skill overlap</p>
                <p><strong>Job Source:</strong> Real-time job data from external APIs</p>
            </div>
        </div>
    </body>
    </html>
...
def upload_file():
    ...
    if file and file.filename.lower().endswith('.pdf'):
        try:
            ...
            job_matches = []
            for job in jobs:
                match_data = calculate_match_score(resume_text, resume_skills, job['description'], job['title'])
                job_matches.append({
                    'job': job,
...
def generate_results_html(resume_skills, experience_years, job_matches):
    ...
    for i, match in enumerate(job_matches[:10], 1):
        ...
        matching_skills_html = ', '.join(match_data['matching_skills']) if match_data['matching_skills'] else 'None'
